...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        self.particles += quotient * self.legalPositions
    ...
    def observe(self, observation, gameState):
        ...
        for pos in self.beliefs:         
            if noisyDistance == None: 
                self.particles = len(self.particles) * [self.getJailPosition()]
                return
            trueDistance = util.manhattanDistance(pos, pacmanPosition)
            self.beliefs[pos] *= emissionModel[trueDistance]

        if self.beliefs.totalCount() != 0:
            self.particles = []
            for i in range(self.numParticles): 
                self.particles += [util.sample(self.beliefs)]
        else: 
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        a belief distribution.
        """
        for idx, prevPos in enumerate(self.particles): 
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, prevPos))
            self.particles[idx] = util.sample(newPosDist)
    ...
    def getBeliefDistribution(self):
        ...
        """
        self.beliefs = util.Counter()
        for p in self.particles: self.beliefs[p] += 1
        self.beliefs.normalize()
        return self.beliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        combinations = []
        combinations += itertools.product(self.legalPositions, repeat=self.numGhosts)
        random.shuffle(combinations)

        length = len(combinations)
        quotient = self.numParticles/length
        remainder = self.numParticles%length

        self.particles = []
        self.particles += quotient * combinations
        self.particles += combinations[:remainder]
        ...
        for particle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] != None:
                    trueDistance = util.manhattanDistance(pacmanPosition, particle[ghost])
                    weight[particle] *= emissionModels[ghost][trueDistance]
                else: 
                    jail = self.getParticleWithGhostInJail(particle, ghost)
                    weight[jail] = weight.pop(particle)
                    particle = jail
            self.beliefs[particle] += weight[particle]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), ghost, self.ghostAgents[ghost])
