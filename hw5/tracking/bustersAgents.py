...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostPosDist in livingGhostPositionDistributions:
            maxBeliefPos = max(ghostPosDist, key=ghostPosDist.get)
            ghosts[maxBeliefPos] = ghostPosDist[maxBeliefPos]
        closestGhostPos = max(ghosts, key=ghosts.get)

        actions = util.Counter()
        for action in legal: 
            successorPos = Actions.getSuccessor(pacmanPos, action)
            actions[action] = self.distancer.getDistance(successorPos, closestGhostPos)
